...
function replaceText(word, replacement, text) {
  const rep = RegExp(word, 'g');
  return text.replace(rep, replacement);
...
function extractTextInParentheses(str) {
  const rep = str.matchAll(/\((.*?)\)/g);
const extra = Array.from(rep, (match) => match[1]);
return extra;
...
function countEmails(str) {
  const reg = str.match(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g);
  return reg.length;
...
if(regex.flags.includes('g') || regex.flags.includes('m')) {
  return true;
} else {
  return false
}
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  return str.replace(reg, newWord);
